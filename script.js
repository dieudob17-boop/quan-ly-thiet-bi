...
const API_URL = "https://script.google.com/macros/s/AKfycbxD3BQvETv6EWGHzp-55PedjGzyU9ro-_5rtaMLtQ3jvj4yw_JLPh2ndMCEX-KNJssr/exec";
...
async function loadData() {
    ...
    try {
       ...
       devices = data.map(d => {
            let reportObj = null;
            // Dịch từ cột Tiếng Việt trên Sheets
            if (d['Lỗi báo cáo'] || d['Ghi chú báo cáo']) {
                reportObj = {
                    issues: d['Lỗi báo cáo'] ? d['Lỗi báo cáo'].split('|') : [],
                    note: d['Ghi chú báo cáo'] || "",
                    time: d['Thời gian báo cáo'] || ""
                };
            }
            // Gán dữ liệu Tiếng Việt vào biến hệ thống
            return {
                id: d['STT'], 
                name: d['Tên thiết bị'], 
                ip: d['Địa chỉ IP'], 
                manufacturer: d['Hãng sản xuất'],
                connectionStatus: d['Tình trạng kết nối'], 
                operationStatus: d['Tình trạng vận hành'],
                type: d['Loại thiết bị'], 
                report: reportObj
            };
        });
        });
